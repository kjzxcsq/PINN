...
if __name__ == '__main__':
    lr_n_list = [0, 10, 15, 20, 25, 30, 50, 75, 100, 300, 500]
    for lr_n_l in lr_n_list:
        print('lr_n: {:5d}'.format(lr_n_l))
        a = 2
        b = 50
        n = 0.1 
        ...
        # x_test_net = (x_test - mu_X) / sigma_X
        y = u(x_test, a, b, n)
        ...
        for steps in range(1, epochs+1):
            ...
            # x_train_net = (x_train - mu_X ) / sigma_X
            # y_1_net = (y_1 - mu_X) / sigma_X
            # y_0_net = (y_0 - mu_X) / sigma_X
            # x_train = torch.autograd.Variable((torch.rand([train_size, 1]) - mu_X) / sigma_X, requires_grad=True).cuda()
            # y_1 = net(((torch.tensor([1.0]) - mu_X ) / sigma_X).cuda())
            # y_0 = net(((torch.tensor([0.0]) - mu_X ) / sigma_X).cuda())
            # print(x_train.mean())
            # print(x_train.std())
            y_hat = net(x_train)
